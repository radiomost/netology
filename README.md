...
## Схема ClusterIP доступа

```mermaid id="arch1"
flowchart LR
    P[Test Pod] --> S[multi-svc ClusterIP]
    S --> N[nginx:80]
    S --> M[multitool:8080]
```
## Запускаем кластер

Используем манифесты [deployment-multi-container.yaml](./deployment-multi-container.yaml) и [service-clusterip.yaml](./service-clusterip.yaml)для запуска кластера
...
```bash
kubectl run test-pod --image=wbitt/network-multitool -it --rm -- sh
```

```bash
curl multi-svc:9001   # nginx
```
...
## Service NodePort (доступ снаружи)

```mermaid
flowchart LR
    Client[Browser / curl] --> NodeIP:30080
...
flowchart TD
    ...
    FP[frontend Pod nginx]
    BP[backend Pod multitool]
...
Файлы манифестов [deployment-frontend.yaml](./deployment-frontend.yaml), [service-frontend.yaml](./service-frontend.yaml)
...
Файлы манифестов [deployment-backend.yaml](./deployment-backend.yaml), [service-backend.yam](./service-backend.yam)
...
Файл манифеста [ingress.yaml](./ingress.yaml)
